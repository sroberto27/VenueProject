...
echo $cname;echo $caddress;echo $cphone;
				$stmt = $conn->prepare( "INSERT INTO agents ( name,phone,email) VALUES ( ?, ?, ?)");
				$stmt->bind_param("sss", $cname, $cphone,$caddress);
				$stmt->execute();
				if($stmt )
				{
		...
		echo $temp1;
				$stmt = $conn->prepare("DELETE FROM agents WHERE (agentID=?)");
	...
	?>
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
			...
			.error{ color: #FF0000;}
				  ...
				  <input class="heighttext" type="text" id="cinfo" name="PNametb" placeholder="agent Name">
				  <input class="heighttext" type="text" id="cinfo" name="PPhtb" placeholder="agent phone">
				  <input class="heighttext" type="text" id="cinfo" name="Paddrtb" placeholder="agent email">
